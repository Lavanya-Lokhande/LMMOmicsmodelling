...
volcano_plots_binary <- function(data, subset = FALSE,
  ...
  if (nrow(p_marker_subset)>1) {
    ...
    gcor <- cor(t(heatmap_input[,s1:e1]), method="pearson")
    plot4 <- corrplot.mixed(gcor,outline = F, order="FPC",addrect = 2, rect.col = "black", rect.lwd = 1, tl.col = "black", tl.cex = 0.5, cl.cex = 0.5, sig.level = 0.05, tl.pos = 'lt', lower = 'color',upper = 'pie')
    print(plot4)
  }
...
volcano_plots_binary_CTA <- function(data, subset = FALSE,
                                 ROI_type,
                                 formula,
                                 ctrl_biom=FALSE,
                                 biom_trans=NULL,
                                 p_sig, coeff_thresh, reflevel, contrastlevel, subset_cat, binary_group, name, s, e, n){
  data <- data
  ...
  plot <- ggplot(p_marker, aes(x=coeff, y=significance, label=biomarker,  fill = factor(threshold), color = factor(threshold))) +
    geom_point(alpha = 0.4, aes(size = significance), show.legend = F) +
    scale_color_manual(values=cols) +
    scale_fill_manual(values=cols) +
    geom_hline(yintercept = -log10(p_sig), linetype="longdash", col="black") +
    geom_vline(xintercept = c(-coeff_thresh, coeff_thresh), linetype="dashed", col="black") +
    geom_text_repel(data = subset(p_marker, FDR<p_sig), size = 2, show.legend = F,
                    point.padding = .3, box.padding = .3,
                    min.segment.length = .2, color="black",
                    max.overlaps = 20) +
    labs(title=paste0(ROI_type," ROI -", " Significant change in ", contrastlevel , " with ", reflevel," as reference and ",'-log[10] ', p_sig," as significance line for ", name),  x="Coefficient", y=expression('-log'[10]*' (FDR adjusted P-value)')) +
    theme(legend.position = "top",legend.text = element_text(size = 7), panel.border = element_rect(color = "black",fill = NA, size = 0.2), plot.title = element_text(size=10), panel.background = element_rect(fill = "white"))
  ...
  if (nrow(p_marker_subset)>1) {
    ...
    plot2 <- Heatmap(as.matrix(scale(mat_scaled)), row_names_gp = gpar(fontsize = 6), cluster_column_slices = TRUE, row_gap = unit(1, "mm"), border = TRUE, row_title_rot = 0, column_names_gp = gpar(fontsize = 0), column_split = data[binary_group], col = pal2(20), top_annotation = ha,  column_title = paste0(ROI_type," ROI -", "Significant change in ", contrastlevel , " with ", reflevel," as reference and ",'-log[10] ', p_sig," as significance line"),left_annotation = pa, show_heatmap_legend = FALSE, cluster_rows = TRUE)
    draw(plot2, show_heatmap_legend = FALSE, show_annotation_legend = FALSE)
   # dev.off()
    print(plot2)
    ...
    plot4 <- corrplot(gcor,outline = F, order="FPC",addrect = 2, rect.col = "black", rect.lwd = 1, tl.col = "black", tl.cex = 0.25, cl.cex = 0.5, sig.level = 0.05, type = "full", method="color")
    print(plot4)
  }
}
